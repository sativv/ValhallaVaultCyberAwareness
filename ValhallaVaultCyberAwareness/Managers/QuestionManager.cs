...
namespace ValhallaVaultCyberAwareness.Managers
{
	public class QuestionManager(ApplicationDbContext context)
	{
		...
		public async Task<double> CalculatePercentageInCategory(int categoryId, ApplicationUser user)
		{
			List<QuestionModel> questions = await uow.QuestionRepo.GetQuestionsInCategoryAsync(categoryId);
		...
		public async Task<double> CalculatePercentageInSubCategory(int subCategoryId, ApplicationUser user)
		{
			List<QuestionModel> questions = await uow.QuestionRepo.GetQuestionsInSubCategoryAsnyc(subCategoryId);
