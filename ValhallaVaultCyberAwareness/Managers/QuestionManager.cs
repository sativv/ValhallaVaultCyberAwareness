...
namespace ValhallaVaultCyberAwareness.Managers
{
	public class QuestionManager(ApplicationDbContext context)
	{
		...
		/// Räknar ut hur många procent rätt man har.
		/// </summary>
		/// <param name="categoryId">Id för subkategorin alla frågor ingår i</param>
		/// <param name="user"></param>
		/// <returns>Antal procentenheter, alltså inte 0.8 utan 80%</returns>
		/// <exception cref="Exception"></exception>
		public async Task<double> CalculateCorrectPercentage(int categoryId, ApplicationUser user)
		{
			List<QuestionModel> questions = await uow.QuestionRepo.GetQuestionsInSubCategoryAsnyc(categoryId);
